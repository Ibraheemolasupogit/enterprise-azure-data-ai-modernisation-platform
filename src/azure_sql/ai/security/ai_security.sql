-- Least-privilege AI role posture. Bind users/groups through Entra in each environment.
CREATE ROLE [ai_app_executor];
CREATE ROLE [ai_data_curator];
CREATE ROLE [ai_auditor];
CREATE ROLE [embedding_worker_identity];

GRANT EXECUTE ON OBJECT::[ai].[usp_AssembleRagContext] TO [ai_app_executor];
